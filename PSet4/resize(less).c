...
int main(int argc, char *argv[])
{
    ...
    int PovWidthSize = (sizeof(RGBTRIPLE)*bi.biWidth);
    int NegWidthSize = -1*(PovWidthSize);
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        for(int u2=0; u2<f;u2++)
        {
            ...
            for (int j = 0; j < bi.biWidth; j++)
            {
                ...
                for(int u3=0; u3<f;u3++)
                    fwrite(&triple, sizeof(RGBTRIPLE), 1, outptr);
            }

            // then add padding to the new file(to demonstrate how)
            for (int k = 0; k < padding2; k++)
                fputc(0x00, outptr);

            // seek back to the starting of the line
            fseek(inptr, NegWidthSize, SEEK_CUR);
        }
        fseek(inptr, PovWidthSize + padding, SEEK_CUR);
